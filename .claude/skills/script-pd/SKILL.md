...
| STRATEGY 실행 시 | + `prompts/pd-templates.md` + `prompts/ctr-reference.md` + `prompts/seo-titles.md` ★제목 A/B/C SSOT★ + `prompts/hook-types.md` ★훅 7유형 SSOT★ + `config/thumbnail-strategy.json` (있으면) |
...
| DRAFT (훅 작성 시) | + `prompts/hook-types.md` (이미 STRATEGY에서 로드된 경우 재로드 불필요) |
| SHORTS 실행 시 | + `prompts/pd-templates.md` (shorts_meta.json 포맷) |
| BREAKING_NEWS_TRACK 진입 시 | + `prompts/news-collect.md` ★뉴스 수집·크로스체크 SSOT★ |
...
### ⚡ 속보 모드 (Breaking News) — 트랙 분기

사용자 요청에 다음 트리거 키워드 1개 이상 포함되면 **속보 트랙**으로 진입한다 (auto/ask 모드와 별개의 가속 트랙):

| 분류 | 트리거 키워드 |
|---|---|
| 직접 표현 | "속보", "긴급", "방금", "지금 막", "터졌어", "터졌다" |
| 발표 인용 | "○○ 발표 났어", "○○ 발표됐어", "○○가 발표했어" |
| 영상 명시 | "속보 대본", "긴급 뉴스 영상", "방금 나온 뉴스로 영상" |

속보 트랙 활성화 시:
- `target_minutes` 묻지 않고 **5~8분 고정** (단일 이슈 집중)
- 레퍼런스 영상 수집(COLLECT) 건너뜀 → WebSearch 자료 수집만
- 파이프라인 단축: WebSearch → 크로스체크 → 단일 DRAFT → REVIEW → SHORTS 1개
- 처리 시간 목표: **15분 이내**
- 상세 절차: `prompts/pd-script.md` § BREAKING_NEWS_TRACK 참조
...
{P}/_script/script.txt 있음 + _script/shorts/shorts_01.txt 없음 + 채널이 쇼츠 ON → SHORTS
{P}/_script/script.txt 있음             → DONE
```

**쇼츠 ON 조건**: `channels/{채널}/config/settings.json`의 `shorts.enabled: true` 또는 사용자가 "쇼츠도", "쇼츠까지" 요청. 기본값은 채널 설정 따름. 명시 OFF 시 SHORTS 건너뜀.
...
| **SHORTS** | _script/shorts/shorts_01~03.txt + shorts_meta.json | PD 직접 (script.txt + verified-data.md 기반 3개 추출) | 60초·170~200자·강한 훅·커버 프롬프트 |
